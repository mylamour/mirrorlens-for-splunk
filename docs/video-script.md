...
**画面：**
Discovery & Evidence is available behind the Show Evidence & Gaps control. Open Agent Trace / MCP Proof to show "Reasoning — Deciding next investigation step" plus `get_indexes`, `get_metadata`, and `run_query` calls with OK status and row counts.

Discovery & Evidence 通过 Show Evidence & Gaps 控制条作为辅助上下文展示。打开 Agent Trace / MCP Proof 抽屉展示推理过程和 MCP 调用状态。
...
**画面 (1:15–1:35):**
Evidence queries are available through Show Evidence & Gaps. Agent Trace / MCP Proof shows multiple `run_query` calls. Iteration counter incrementing.
